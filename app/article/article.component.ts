...
export class ArticleDetailComponent implements OnInit{
    ...
    // Supprimer tout les données enregistré
    private supprimerTout() {
        this._listeArticles.splice(0);
        this.enrArticle = new Article(-1,'',0);
    }
    ...
    private setSuivant() {
        if (this._indiceEnCours < this._listeArticles.length - 1)
            this.setEncours(++this._indiceEnCours); //suivant
    }

    private setPrecedent() {
        if (this._indiceEnCours > 0)
            this.setEncours(--this._indiceEnCours); //precedent
    }
